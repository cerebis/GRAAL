...
def build_and_filter(base_folder, size_pyramid, factor):
    ...
    # print "the file is : ", hdf5_pyramid_file
    encode_sys_file = sys.getfilesystemencoding()
    # if encode_sys_file == None:
    #     print "No encoding system"
    # else:
    #     print "Yesss it is bitch!!"
    ...
    for level in xrange(0, size_pyramid):
        ...
        if level>0:
            ...
            # if os.path.exists(new_contig_list_file) and os.path.exists(new_fragments_list_file) and os.path.exists((sub_2_super_frag_index_file)):
            ...
            if os.path.exists(new_contig_list_file) and os.path.exists(new_fragments_list_file) and os.path.exists(new_abs_fragments_contacts_file):
            # if os.path.exists(new_contig_list_file) and os.path.exists(new_fragments_list_file) :
                print "level already built..."
                nfrags = file_len(new_fragments_list_file) - 1
...
def abs_contact_2_coo_file(abs_contact_file, coo_file):
    ...
    for i in range(1, n_lines):
        ...
        if i%10**6 == 0:
            p.render(pt * 100, 'step %s\nProcessing...\nDescription: convert dense file to COO sparse data.' % i)
        line = all_lines[i]
        dat = line.split()
